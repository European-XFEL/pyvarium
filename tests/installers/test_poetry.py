...
class TestPoetry:
    ...
    def poetry_home(self, tmp_path_factory) -> Path:  # type: ignore
    ...
    def test_setup_no_force(self, poetry_home: Path):
        process = Poetry.setup(poetry_home)
        assert "is already installed" in process.stdout.decode()
